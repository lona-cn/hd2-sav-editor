...
> 当前维护提示：本文是初始交接背景，不代表项目仍未实现。Rust 与 Python 已支持 Observed0106 / Observed0107
> 两组精确布局，不能把旧头部/长度当成唯一格式，也不能全局替换旧夹具。
> 当前支持规则见根 README，实际验证见 `hd2-armor-desk/docs/TEST_RECORD.md`；旧备份继续支持，未知布局仍只读。
