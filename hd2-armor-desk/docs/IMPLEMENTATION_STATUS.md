...
- Rust / Python 均精确支持 Observed0106 和 Observed0107，混配头部/长度与其他未知布局继续只读。
- 保存保留当前布局、非目标字段和未知尾部；旧草稿不能安全覆盖新磁盘内容，显式 force-latest 只重放装备意图。
- 新增合成 0107 夹具、双向跨语言互验及严格缺输出检查；不把真实存档提交为夹具。
- Python 旧默认字段标签定向迁移，自定义设置保留。目录 v2 增加副武器类型；最后通牒保守迁移，不覆盖用户确认或合并冲突项。
- 本轮 Rust 128 项、Python 48 项通过；Clippy 零警告；真实新样本离线往返和 Tk / 原生桌面读取已验证。
- 远端 CI、release 打包和游戏内验收未在本轮执行。详细命令、环境差异和证据见 `TEST_RECORD.md`。
...
| `app` | GPUI 界面：双槽卡片（身体默认解锁）、搜索、装备库护甲被动详情、导入预览、预设、差异栏、双写回模式、GitHub 更新检查与下载进度条、诊断与后台任务 |
